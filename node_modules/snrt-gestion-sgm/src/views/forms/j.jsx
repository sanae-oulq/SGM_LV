...
const FormsElements = () => {
  ...
  const handleFactureFileDownloadClick = async (facture) => {
    try {
      ...
      setFactureValidationMessage("Erreur lors du téléchargement du fichier.");
      setShowFactureValidationModal(true);
    }
  };
...
{/**nv pvrpt */}
  ...
  const handleDownloadBRIs = (fileEntry) => {
  ...
  const handleUploadBRIs = () => {
    ...
    console.log('Fichier sélectionné:', selectedFileBRIs);
  
    const newFileEntry = {
      id: fileListBRIs.length + 1,
      fileName: selectedFileBRIs.name,
      date: new Date().toLocaleDateString(),
      file: selectedFileBRIs,
      marcheBC: marcheBC
    };
  ...
  const handleSaveBRIs = async () => {
    ...
    if (fileListBRIs.length === 0) {
      setBrisValidationMessage("Aucun fichier à enregistrer !");
      setShowBrisValidationModal(true);
      return;
    }

    try {
      // Récupérer d'abord les détails du marché
      ...
      // Garder une trace des fichiers réussis, échoués et existants
      const successfulFiles = [];
      const failedFiles = [];
      const existingFiles = [];

      // Save each file in the list
      for (const fileEntry of fileListBRIs) {
        try {
          ...
          if (checkResult.exists) {
            existingFiles.push(fileEntry.fileName);
            continue;
          }
          ...
          const response = await fetch('http://localhost:5003/api/bris', {
            method: 'POST',
            body: formData
          });
          ...
          if (!response.ok) {
            const errorData = await response.json();
            console.error('Réponse du serveur en erreur:', errorData);
            failedFiles.push(fileEntry.fileName);
          } else {
            successfulFiles.push(fileEntry.fileName);
          }
        } catch (fileError) {
          console.error("Erreur détaillée lors de l'enregistrement du fichier:", fileError);
          failedFiles.push(fileEntry.fileName);
        }
      }
      ...
      if (existingFiles.length > 0) {
        message += `${existingFiles.length} fichier(s) existe(nt) déjà : ${existingFiles.join(', ')}. `;
      }
      if (successfulFiles.length > 0) {
        message += `${successfulFiles.length} fichier(s) enregistré(s) avec succès : ${successfulFiles.join(', ')}. `;
      }
      if (failedFiles.length > 0) {
        message += `\nÉchec pour ${failedFiles.length} fichier(s) : ${failedFiles.join(', ')}`;
      }

      setBrisValidationMessage(message);
      setShowBrisValidationModal(true);
  ...
  const handleValidate = async () => {
    ...
    try {
      const response = await attestationConformiteService.upload(selectedFile, marcheBC);
 ...
 const [pvpFormData, setPvpFormData] = useState({
  ...
  date: "",
  ficheProjet: null,
  marcheBCType: "marche" // Ajout du type pour PV provisoire
});
...
const [pvdFormData, setPvdFormData] = useState({
  ...
  date: "",
  ficheProjet: null,
  marcheBCType: "marche" // Ajout du type pour PV définitif
});
...
const handlePVPSubmit = () => {
  if (pvpFormData.date && pvpFormData.ficheProjet) {
    ...
    setPvpFormData({
      ...
      date: "",
      ficheProjet: null,
      marcheBCType: "marche" // Conserver le type par défaut
    });
  } else {
...
*/}
  ...
  const handlePvSubmit = async () => {
    ...
    try {
      const marcheResponse = await fetch(`http://localhost:5003/api/amarche/${marcheBC}`);
      ...
      if (showEditPvModal) {
        setPvList(pvList.map(pv => pv === selectedPv ? { ...pvFormData, document: pvFormData.document || "Aucun" } : pv));
        setValidationMessage("PVRPT modifié avec succès.");
      } else {
        setPvList([...pvList, { ...pvFormData, document: pvFormData.document || "Aucun" }]);
