...
const FormsElements = () => {
  ...
  const handleSaveFacture = () => {
    ...
    if (selectedFacture) {
      setFactureList((prevList) =>
        prevList.map((facture) =>
          facture === selectedFacture ? { ...factureFormData, fichier: factureFormData.fichier ? factureFormData.fichier.name : "Aucun" } : facture
        )
      );
      setFactureValidationMessage("Facture modifiée avec succès.");
    } else {
      setFactureList([...factureList, { ...factureFormData, fichier: factureFormData.fichier ? factureFormData.fichier.name : "Aucun" }]);
  ...
  const confirmFactureDelete = () => {
    setFactureList(factureList.filter((facture) => facture !== selectedFacture));
    setSelectedFacture(null);
...
{/**nv pvrpt */}
  ...
  const handleSaveOrdreService = async () => {
    ...
    try {
      ...
      if (showEditOrdreServiceModal) {
        ...
        savedOrderService = await ordreServiceService.create(ordreServiceFormData);
  ...
  const [selectedFileBRIs, setSelectedFileBRIs] = useState(null);

  // Gérer la sélection de fichier
  const handleFileSelectBRIs = (e) => {
    setSelectedFileBRIs(e.target.files[0]);
  };
  ...
  const handleUploadBRIs = () => {
    ...
    const newFileEntry = {
      ...
      fileName: selectedFileBRIs.name,
      date: new Date().toLocaleDateString(),
      fileData: selectedFileBRIs,
    };

    setFileListBRIs([...fileListBRIs, newFileEntry]);
    setSelectedFileBRIs(null);
  };

  // Supprimer un fichier du tableau
  const handleRemoveBRIs = (id) => {
    setFileListBRIs(fileListBRIs.filter((file) => file.id !== id));
  };

  // Télécharger un fichier
  const handleDownloadBRIs = (file) => {
  ...
  // Enregistrer les données (exemple de console log)
  const handleSaveBRIs = () => {
    if (fileListBRIs.length === 0) {
      alert("Aucun fichier à enregistrer !");
      return;
    }
    console.log("Données enregistrées :", fileListBRIs);
  ...
  const handleFileSelect = (e) => {
    setSelectedFile(e.target.files[0]); // Stocke le fichier sélectionné
  };
  ...
  const handleValidate = () => {
    ...
    alert(`Fichier "${selectedFile.name}" validé avec succès !`);
 ...
 const [pvpFormData, setPvpFormData] = useState({
  id: "1", // ID désactivé (peut être généré dynamiquement)
  date: "",
  ficheProjet: null,
});
...
const [pvdFormData, setPvdFormData] = useState({
  id: "2", // ID désactivé (peut être généré dynamiquement)
  date: "",
  ficheProjet: null,
});
...
const handlePVPSubmit = () => {
  if (pvpFormData.date && pvpFormData.ficheProjet) {
    ...
    console.log("Données soumises PVP :", pvpFormData);
...
const handlePVDSubmit = () => {
  ...
  const handleFichesSubmit = (e) => {
    e.preventDefault();
    if (
      fichesFormData.intituleProjet &&
      fichesFormData.dateProjet &&
      fichesFormData.chefProjet &&
      fichesFormData.commission &&
      fichesFormData.ficheProjet &&
      fichesFormData.ficheAffectation
    ) {
      alert("Formulaire soumis avec succès !");
      // Ici vous pouvez gérer l'envoi du formulaire (par exemple, à une API)
      console.log(fichesFormData);
...
*/}
  ...
  const handlePvSubmit = () => {
    if (!pvFormData.datePVRPT) {
      setValidationMessage("Tous les champs obligatoires doivent être remplis.");
      setShowValidationModal(true);
      return;
    }
      
    const fileData = pvFormData.document ? {
      name: pvFormData.document.name,
      type: pvFormData.document.type,
      data: pvFormData.document
    } : null;
      
    if (showEditPvModal) {
      setPvList(pvList.map(pv => (pv === selectedPv ? { ...pvFormData, document: fileData || "Aucun" } : pv)));
      setValidationMessage("PVRPT modifié avec succès.");
    } else {
      setPvList([...pvList, { ...pvFormData, document: fileData || "Aucun" }]);
      setValidationMessage("PVRPT ajouté avec succès.");
    }
  ...
  const confirmPvDelete = () => {
    setPvList(pvList.filter((pv) => pv !== selectedPv)); // Supprimer la ligne sélectionnée
    setSelectedPv(null);
...
const handleValider = () => {
  ...
  const filteredFamilleProjetData = [
    { code: 'FP1', designation: 'Famille Projet A' },
    { code: 'FP2', designation: 'Famille Projet B' },
    { code: 'FP3', designation: 'Famille Projet C' },
  ].filter(item =>
    Object.values(item).some(val =>
  ...
  const filteredFicheProjetData = [
    { id: 'FP001', title: 'Projet Construction Alpha', dateProjet: '2023-05-15', chefProjet: 'Ahmed Khalid', Commission: 'Commission A', ficheProjetFile: 'doc1.pdf', type: 'Construction', ficheAffectationFile: 'aff1.pdf' },
    { id: 'FP002', title: 'Projet Rénovation Beta', dateProjet: '2023-06-20', chefProjet: 'Sara Mansour', Commission: 'Commission B', ficheProjetFile: 'doc2.pdf', type: 'Rénovation', ficheAffectationFile: 'aff2.pdf' },
    { id: 'FP003', title: 'Projet Maintenance Gamma', dateProjet: '2023-07-10', chefProjet: 'Mohammed Ali', Commission: 'Commission C', ficheProjetFile: 'doc3.pdf', type: 'Maintenance', ficheAffectationFile: 'aff3.pdf' },
  ].filter(item =>
    Object.values(item).some(val =>
  ...
  useEffect(() => {
    ...
    if (state && state.isEdit && state.marche) {
      ...
      setSelectedFamilleProjetCode(state.marche.famille || '');
      ...
      fetchMarcheDetails(state.marche.marche);
    }
  }, [location]);
  ...
  const handleSaveDetailsPrix = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5003/api/amarches/${selectedMarcheDetails.marcheBC}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
                                ...
                                {pv.document !== "Aucun" ? (
                                  <Button 
                                    ...
                                    onClick={(e) => {
                                      ...
                                      if (pv.document && pv.document.data) {
                                        const url = URL.createObjectURL(pv.document.data);
                                        const a = document.createElement('a');
                                        a.href = url;
                                        a.download = pv.document.name;
                                        document.body.appendChild(a);
                                        a.click();
                                        document.body.removeChild(a);
                                        URL.revokeObjectURL(url);
